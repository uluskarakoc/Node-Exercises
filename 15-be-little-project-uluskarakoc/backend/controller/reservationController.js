...
export const getReservation = async (req, res) => {
  ...
  const reservation = await ReservationModel.findById(id).populate('roomNumber');
  if (reservation) {
...
export const addReservation = async (req, res) => {
  ...
  reservation.roomNumber = roomId;

  try {
    const room = await RoomModel.findById(roomId);
    if (!room) {
      return res.status(404).send(`Room hat nicht gefunden. ID: ${roomId}`);
    }
